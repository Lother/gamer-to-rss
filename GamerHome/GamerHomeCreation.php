<?php
class GamerHomeCreation extends GamerHome {
	protected $_creation_url = 'http://home.gamer.com.tw/creation.php?v=1&owner=';
	protected $_title = ' 的小屋創作 RSS';
	protected $_find_page = '的創作';
			...
			foreach($matches[1] as $i => $id){
				...
				if (preg_match_all('#reply_div_([^"]+)[^<]+<[^<]+<[^<]+<[^<]+<[^<]+<A class="msgname AT1" href="([^=]+=)([^"]+)">([^<]+)[^>]+>[^>]+>(.+?)<SPAN class="msgtime ST1">(:?(\d{2}-\d{2} \d{2}:\d{2})|([^天<]+)天(\d{2}:\d{2})|(\d{1,2})([^前<]+)前|1分內)<\/SPAN>#isu',$html2,$matches2))
				{
					foreach($matches2[1] as $j => $cid ){
						if($this->_filter==''||$this->_filter==$matches2[3][$j])
						{
							$tmp2 = array(
								'id' => $cid,
